...
describe("extractTableMetadata", () => {
  ...
  it("extracts the bundled sample database to YAML", async () => {
    const stats = await extractTableMetadata({
  ...
  it("strips numeric ids and uses natural-key db_id on tables", async () => {
  ...
  it("rewrites fk_target_field_id as a natural-key tuple", async () => {
    await extractTableMetadata({
      inputFile: EXAMPLE_INPUT,
      outputFolder: workdir,
    });
  ...
  it("escapes slashes in entity names", async () => {
    const input = join(workdir, "input.json");
    writeFileSync(
      input,
      JSON.stringify({
        databases: [{ id: "weird/name", name: "weird/name", engine: "h2" }],
        tables: [],
        fields: [],
      }),
    );
    const out = join(workdir, "out");
    await extractTableMetadata({ inputFile: input, outputFolder: out });
  ...
  it("regenerates output that matches the bundled examples", async () => {
    await extractTableMetadata({
      inputFile: EXAMPLE_INPUT,
      outputFolder: workdir,
    });
